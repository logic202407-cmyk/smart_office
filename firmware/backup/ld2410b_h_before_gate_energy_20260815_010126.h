...
// ============================================================
// 功能说明：
//   1. 字节级状态机解析 LD2410B 上报帧 (F4F3F2F1 ... F8F7F6F5)
//   2. 支持粘包/半包/错帧恢复
//   3. UART2 初始化 (256000 8N1)
//   4. 配置命令发送 (使能配置/读取版本/工程模式)
// ============================================================
#ifndef LD2410B_H
#define LD2410B_H
...
// 目标状态值
#define LD2410B_TARGET_NONE       0x00  // 无目标
#define LD2410B_TARGET_MOVING     0x01  // 运动目标
...
/**
 * @brief LD2410B 雷达目标信息结构体
 *        由解析层填充，供上层状态机使用
 */
struct LD2410B_TargetInfo {
    uint8_t  target_state;      // 目标状态: 0=无, 1=运动, 2=静止, 3=运动+静止
    ...
    uint32_t timestamp;         // 最近一次有效帧时间戳 (ms)
    bool     valid;             // 当前数据是否有效 (true=雷达数据已更新)
};
...
enum LD2410B_ParserState : uint8_t {
    ...
    PARSER_WAIT_LEN_L,          // 已收帧头, 等待长度低字节
    PARSER_WAIT_LEN_H,          // 已收长度低, 等待长度高字节
    PARSER_WAIT_DATA,           // 正在读取数据负载
    ...
    // ---- 配置帧解析 (补充支持) ----
    PARSER_CFG_GOT_FD,          // 已收0xFD (配置帧起始), 等待0xFC
    PARSER_CFG_GOT_FC,          // 已收0xFDFC, 等待0xFB
    PARSER_CFG_GOT_FB,          // 已收0xFDFCFB, 等待0xFA
    PARSER_CFG_WAIT_LEN_L,      // 已收配置帧头, 等待长度低
    PARSER_CFG_WAIT_LEN_H,      // 等待长度高
    PARSER_CFG_WAIT_DATA,       // 读取配置帧负载
    PARSER_CFG_WAIT_04,         // 等待配置帧尾0x04
...
public:
    ...
    /**
     * @brief 初始化 UART2 并重置解析器
     * @param serial 可选: 指定 HardwareSerial 实例 (默认 &Serial2)
     * @return true  初始化成功
     */
    bool begin(HardwareSerial* serial = nullptr);

    /**
     * @brief 向解析器送入一个字节 (字节级状态机)
     * @param byte 从 UART 读取的原始字节
     */
    void feed(uint8_t byte);

    /**
     * @brief 从 UART 读取所有可用数据并送入解析器
     *        需在 loop() 中高频调用
     */
    void update();
    ...
    /**
     * @brief 使能配置模式 (发送 0xFF 0xFF 0x00 0x01)
     *        配置命令必须在使能配置后才有效
     */
    void enableConfig();
    ...
    /**
     * @brief 设置/关闭工程模式
     * @param enable true=开启工程模式 (更多数据), false=关闭
     */
...
private:
    ...
    LD2410B_ParserState _state;             // 当前状态机状态
    LD2410B_TargetInfo  _info;              // 最新解析结果
    uint32_t            _frame_count;       // 成功解析帧计数器

    // ---- 帧接收缓冲区 ----
    uint8_t             _buf[128];          // 数据负载缓冲区
    uint16_t            _buf_index;         // 当前缓冲区写入位置
    uint16_t            _frame_len;         // 预期负载长度 (从帧中解析)
