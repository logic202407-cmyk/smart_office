...
static uint16_t selectPrimaryDistanceMm(const LD2410B_TargetInfo& info)
{
    ...
    // detect_dist is the closest gate with energy — it gets stuck on nearby
    ...
    if (info.target_state == LD2410B_TARGET_BOTH)
    {
        ...
        // continuity — no sudden jumps to a different person).
        if (info.static_dist > 0 && info.moving_dist > 0)
        {
            ...
            // Threshold: 300 → 30%. Floor at 200mm so tiny gaps don't flip.
            if (gap > max(larger * 3U / 10U, 200U))
            {
                // Two different people — lock onto the distance closer to
        ...
        // Static is the person's settled position — prefer it.
...
static void restoreWireBus()
{
    ...
    // No setTimeOut — let U8g2 I2C transactions complete naturally
}
